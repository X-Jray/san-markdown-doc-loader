...
module.exports = function (md, name, options) {
    var _Object$assign = Object.assign({}, DEFAULT_OPTIONS, options),
        validate = _Object$assign.validate,
        render = _Object$assign.render,
        marker = _Object$assign.marker;
    ...
    function fence(state, startLine, endLine, silent) {
        ...
        var markup = state.src.slice(begin, begin + MARKER_LEN);
        var params = state.src.slice(begin + MARKER_LEN, end);
        ...
        for (; nextLine <= endLine; nextLine++) {
            ...
            var _begin = state.bMarks[nextLine] + state.tShift[nextLine];
            var _end = state.eMarks[nextLine];
            var mem = _begin;

            if (_begin < _end && state.sCount[nextLine] < state.blkIndent) {
                break;
            }

            if (state.src.charAt(_begin) !== marker) {
                continue;
            }
            ...
            _begin = state.skipChars(_begin, marker.charCodeAt(0));

            if (_begin - mem < MARKER_LEN) {
                continue;
            }

            _begin = state.skipSpaces(_begin);
        ...
        var token = state.push(name, 'div', 0);
